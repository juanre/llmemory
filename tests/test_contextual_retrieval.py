...
async def test_contextual_retrieval_improves_precision(test_db_factory):
    ...
    memory_no_context = LLMemory(
        connection_string=db_no_context.config.get_dsn(), config=config_no_context
    )
    ...
    results_no_context = await memory_no_context.search(
        owner_id="test", query_text="Q3 technology revenue growth", limit=5
    )
    ...
    memory_with_context = LLMemory(
        connection_string=db_with_context.config.get_dsn(), config=config_with_context
    )
    ...
    if results_no_context and results_with_context:
        assert (
            results_with_context[0].score >= results_no_context[0].score * 0.9
